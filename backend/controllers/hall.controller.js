import Hall, { find } from '../models/Hall';
...
export async function addHall(req, res) {
  const { hall_name, capacity, location, description, availability_status, support_document } = req.body;
  try {
    const hall = new Hall({ hall_name, capacity, location, description, availability_status, support_document });
    await hall.save();
